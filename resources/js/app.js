        ...
        <a href="${e.link}" target="_blank">
            <div class="media" style="background: #fafb8f9e">
                ...
                <div class="media-body">
                    <h6 class="media-heading">${e.title}</h6>
                    <p class="notification-text font-small-3 text-muted">${e.message}</p>
                    <small>
                        <time class="media-meta text-muted date-time" datetime="${e.created_at}"></time>
                    </small>
                </div>
            </div>
        </a>
...
Echo.channel("outOfStock")
    .listen("OutOfStockEvent", (e) => {
        // console.log(e);
        ...
        <a href="${e.link}" target="_blank">
            <div class="media" style="background: #fafb8f9e">
                <div class="media-left align-self-center"><i
                        class="ft-plus-square icon-bg-circle bg-cyan"></i></div>
                <div class="media-body">
                    <h6 class="media-heading">نفاذ كمية منتج</h6>
                    <p class="notification-text font-small-3 text-muted">عدد المنتجات : ${e.products}</p>
                    <small>
        ...
        if (Notification.permission === "granted") {
            // Display the notification
            const notification = new Notification("نفاذ كمية منتج", {
                body: "عدد المنتجات : " + e.products,
                data: {
                    url: e.link, // Replace with your custom page URL
                },
            });
